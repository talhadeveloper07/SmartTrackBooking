...
@section('business_content')
    <div class="container">
        ...
        <div class="d-flex align-items-center mb-3">
            <h3 class="me-auto">Edit Employee — {{ ucwords($business->name) }}</h3>
        ...
        <form action="{{ route('business.employees.update', [$business->slug, $employee->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @php
                        $employeeServiceIds = isset($employee_services)
                            ? $employee_services
                            : (method_exists($employee, 'services') ? $employee->services->pluck('id')->toArray() : []);

                        $oldServices = old('services', $employeeServiceIds);
                    @endphp
                    ...
                    <div class="service-list">
                        @forelse($services as $srv)
                            ...
                            <div class="service-item">
                                <input type="checkbox" class="service-check" name="services[]" id="srv_{{ $srv->id }}"
                                    value="{{ $srv->id }}" {{ $checked ? 'checked' : '' }}>
            ...
            @php
                ...
                $employeeHours = $employeeHours ?? ($employee->hours ?? []);
                $oldHours = old('hours', $employeeHours);
            @endphp
                ...
                <div class="card-body p-0">
                    @foreach($days as $dayIndex => $dayName)
                        ...
                        @php
                            ...
                            // ✅ FIX: In edit, if day not saved, keep OFF by default
                            $isEnabled = $dayOld ? !empty($dayOld['is_enabled']) : 0;

                            // ✅ FIX: If disabled, slots can still exist but editor will remain hidden
                            $slots = $dayOld['slots'] ?? [
                            ...
                            <div class="d-flex align-items-center justify-content-between px-4 py-3">
                                <div class="d-flex align-items-center gap-3">
                                    <label class="switch mb-0">
                                        {{-- must be BEFORE checkbox --}}
                                        <input type="hidden" name="hours[{{ $dayIndex }}][is_enabled]" value="0">

                                        <input type="checkbox" class="day-toggle" data-day="{{ $dayIndex }}"
                                            name="hours[{{ $dayIndex }}][is_enabled]" value="1" {{ $isEnabled ? 'checked' : '' }}>
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <button type="button" class="btn btn-link p-0 text-primary edit-day"
                                        data-bs-toggle="collapse" data-bs-target="#editDay{{ $dayIndex }}">
                                        <i class="fa fa-pen"></i>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="collapse" id="editDay{{ $dayIndex }}">
                                <div class="px-4 pb-4">
                                    <div class="schedule-editor rounded-3 border p-3 {{ !$isEnabled ? 'd-none' : '' }}"
                                                    ...
                                                    <div class="row g-2 align-items-end">
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <div class="d-flex align-items-center gap-2">
                                                                <input type="text" class="form-control time-text time-start"
                                                                    placeholder="10:30" style="max-width:110px;">
                                                            ...
                                                            <input type="hidden" class="hidden-start"
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <div class="d-flex align-items-center gap-2">
                                                                <input type="text" class="form-control time-text time-end"
                                                                    placeholder="05:00" style="max-width:110px;">
                                                                ...
                                                                <button type="button"
                                                                    class="btn btn-sm btn-outline-danger remove-slot ms-auto">
                                                                    <i class="fa fa-times"></i>
                                                                </button>
                                                            </div>

                                                            <input type="hidden" class="hidden-end"
                                        ...
                                        <button type="button" class="btn btn-outline-primary w-100 mt-3 add-slot"
                                            data-day="{{ $dayIndex }}" data-dayname="{{ $dayName }}">
    ...
    <script>
        ...
        document.addEventListener('change', function (e) {
            ...
            if (editor) {
                editor.classList.toggle('d-none', !isEnabled);

                // ✅ FIX: disable all inputs inside editor when OFF (so slots won’t submit)
    ...
    <script>
        (function () {
            ...
            function syncSelectAll() {
                const list = checks();
                if (!list.length || !selectAll) return;
